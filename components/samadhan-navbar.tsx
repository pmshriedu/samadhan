...
export default function SamadhanNavbar() {
  ...
  // Check auth state from mockAuth
  const authSession = mockAuth.getSession();
  const session: SamadhanSession | null = authSession
    ? { name: authSession.name, phone: authSession.phone }
    : null;
